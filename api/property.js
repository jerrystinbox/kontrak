...
const CONFIG = {   
    gg55:    { BusinessID: "GG",    pageTitle: "Kontrak Kantor Hijau 🔒︎", db: process.env.NOTION_DB_PROPERTY_KANTOR, pinAdmin: process.env.PIN_ADMIN, pinInsert: process.env.PIN_ADMIN },
    gg22:    { BusinessID: "GG",    pageTitle: "Kontrak Kantor Hijau 🔍︎", db: process.env.NOTION_DB_PROPERTY_KANTOR, pinAdmin: "2095231",             pinInsert: "2095231" },
    gg42:    { BusinessID: "GG",    pageTitle: "Kontrak Kantor Hijau",    db: process.env.NOTION_DB_PROPERTY_KANTOR, pinAdmin: "251011",              pinInsert: "251011" }
};
...
function mapPageProperties(page, type) {
    ...
    return {
        ...
        daerah: props.DAERAH?.select?.name || "",
        kategori: props.KATEGORI?.select?.name || "",
        alamat: props.ALAMAT?.rich_text?.map(t => t.plain_text).join("") || "",
        catatan: props.CATATAN?.rich_text?.map(t => t.plain_text).join("") || props.KETERANGAN?.rich_text?.map(t => t.plain_text).join("") || "",
        dokumen: docs,
...
async function buildPropertiesData(notion, targetId, databaseId, fields) {
    ...
    const { 
        rec_id, nama, status, daerah, kategori, alamat, catatan, formattedFiles 
    } = fields;
    ...
    // 4. DAERAH (select / rich_text)
    if (propSchema['DAERAH'] && daerah !== undefined) {
        const type = propSchema['DAERAH'].type;
        if (type === 'select') {
            propertiesData['DAERAH'] = daerah ? { select: { name: daerah } } : { select: null };
        } else if (type === 'rich_text') {
            propertiesData['DAERAH'] = { rich_text: [{ text: { content: daerah || "" } }] };
        }
    }

    // 5. KATEGORI (select / rich_text)
    if (propSchema['KATEGORI'] && kategori !== undefined) {
        const type = propSchema['KATEGORI'].type;
        if (type === 'select') {
            propertiesData['KATEGORI'] = kategori ? { select: { name: kategori } } : { select: null };
        } else if (type === 'rich_text') {
            propertiesData['KATEGORI'] = { rich_text: [{ text: { content: kategori || "" } }] };
        }
    }

    // 6. ALAMAT (rich_text)
    if (propSchema['ALAMAT'] && alamat !== undefined) {
        propertiesData['ALAMAT'] = { rich_text: [{ text: { content: alamat || "" } }] };
    }

    // 7. CATATAN (rich_text)
    const catatanKey = propSchema['CATATAN'] ? 'CATATAN' : (propSchema['KETERANGAN'] ? 'KETERANGAN' : null);
    if (catatanKey && catatan !== undefined) {
        propertiesData[catatanKey] = { rich_text: [{ text: { content: catatan || "" } }] };
    }
...
export default async function handler(req, res) {
    ...
    if (req.method === 'POST' && req.query.action !== 'create' && req.query.action !== 'blob-auth') {
        try {
                ...
                words.forEach(word => {
                    filterConditions.push({
                        or: [
                            ...
                            { property: 'ALAMAT', rich_text: { contains: word } },
                            { property: 'CATATAN', rich_text: { contains: word } }
                        ]
                    });
                });
            }
            ...
            data.sort((a, b) => {
                ...
                if (sortBy === 'daerah') return (a.daerah || "").trim().localeCompare((b.daerah || "").trim(), 'id', { sensitivity: 'base' });
                if (sortBy === 'kategori') return (a.kategori || "").trim().localeCompare((b.kategori || "").trim(), 'id', { sensitivity: 'base' });
    ...
    if (req.method === 'PUT' || (req.method === 'POST' && req.query.action === 'create')) {
        ...
        const daerah = getSingleValue(bodyFields.daerah);
        const kategori = getSingleValue(bodyFields.kategori);
        const alamat = getSingleValue(bodyFields.alamat);
        const catatan = getSingleValue(bodyFields.catatan) || getSingleValue(bodyFields.keterangan);
